...
void reverse_data(NODE *hptr)
{
	...
	/*
	- Preparing the middle node, So that we can use that in traversing second part(last) of link list.
	- So that we no need to travers every time to last node or second last every.., We are starting from the 
	  middle node so it will improve performance
	*/
	...
	for(int i = 0; i < middleCount; i++){
		  ...
		  lastNODE = middleNode;
		  /* 
		  - With below login we are starting from middle next node and reaching to last node.
		  - Next time we will reach second last node.
		  - This way we will swap first and last, second and second last node.
		  */ 
